...
mod tests {
    ...
    fn default_values() {
        ...
        assert_eq!(c.last_device_id, "mock");
        assert_eq!(c.default_dpi, 150);
        assert_eq!(c.default_width, 320);
        assert_eq!(c.default_height, 240);
        assert_eq!(c.levels_white, 255);
        assert!((c.levels_gamma - 1.0).abs() < f64::EPSILON);
        assert_eq!(c.saturation, 0.0);
        assert_eq!(c.hue, 0.0);
        assert!(c.curves.is_none());
        assert!(c.crop.is_none());
    }

    #[test]
    fn curve_points_require_ordered_in_range_pairs() {
        assert_eq!(
            parse_curve_points("0:0, 128:160, 255:255").unwrap(),
            Some(vec![[0, 0], [128, 160], [255, 255]])
        );
        assert!(parse_curve_points("0:0,255:256").is_err());
        assert!(parse_curve_points("128:128,0:0").is_err());
        assert!(parse_curve_points("0:0").is_err());
        assert!(validate_saturation(101.0).is_err());
        assert!(validate_hue(-181.0).is_err());
    }

    #[test]
    fn output_name_accepts_one_portable_filename_component() {
        for valid in ["scan", "scan-01", "Résumé 2026", "report.v1"] {
            assert_eq!(validate_output_name(valid).unwrap(), valid);
        }
        for invalid in [
            "",
            ".",
        ...
        assert!(validate_output_name(&"a".repeat(MAX_OUTPUT_NAME_BYTES + 1)).is_err());
    }

    #[test]
    fn invalid_output_name_is_rejected_on_load_and_before_save_mutation() {
        let dir = env::temp_dir().join(format!("open_scanline_output_name_{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("config.json");
        std::fs::write(&path, "config sentinel").unwrap();

        let invalid = AppConfig {
            output_name: "../outside".into(),
            ..AppConfig::default()
        };
        assert!(save_config(&invalid, &path).is_err());
        assert_eq!(std::fs::read_to_string(&path).unwrap(), "config sentinel");

        std::fs::write(&path, r#"{"output_name":"NUL.txt"}"#).unwrap();
        assert!(load_config(Some(&path)).is_err());
    }

    #[test]
    fn banned_keys_stripped_on_load() {
        let raw = json!({
            "last_device_id": "wia:1",
            "default_dpi": 300,
            "activation": "SECRET",
            "activation_key": "AK",
            "ActivationKey": "AK2",
            ...
            "brightness": 10,
            "nested": {
                "DRM_KEY": "nested secret",
                "safe": "kept",
            },
        });
        let cleaned = strip_banned(&raw);
        assert_eq!(cleaned["nested"], json!({ "safe": "kept" }));
        let cfg = config_from_value(raw).expect("config_from_value");
        assert_eq!(cfg.last_device_id, "wia:1");
        assert_eq!(cfg.default_dpi, 300);
        assert_eq!(cfg.brightness, 10);
        let v = serde_json::to_value(&cfg).unwrap();
        if let Value::Object(map) = v {
            for k in map.keys() {
                assert!(!is_banned_key(k), "banned key leaked: {k}");
            }
        }
    }

    #[test]
    fn json_roundtrip_save_load_strips_banned() {
        let dir = env::temp_dir().join("open_scanline_config_test");
        ...
        let cfg = AppConfig {
            ...
            saturation: 22.5,
            hue: -45.0,
            curves: Some(vec![[0, 0], [128, 160], [255, 255]]),
            crop: Some([1, 2, 3, 4]),
            auto_orient: true,
            auto_crop: true,
        ...
        if let Value::Object(ref mut map) = injected {
            map.insert("license_key".into(), json!("SHOULD_NOT_LOAD"));
            map.insert("DRM".into(), json!(1));
            map.insert("activation_key".into(), json!("x"));
        }
        std::fs::write(&path, serde_json::to_string_pretty(&injected).unwrap()).unwrap();

        let loaded = load_config(Some(&path)).expect("load");
        assert_eq!(loaded.default_dpi, 200);
        assert_eq!(loaded.brightness, -5);
        assert_eq!(loaded.saturation, 22.5);
        assert_eq!(loaded.hue, -45.0);
        assert_eq!(loaded.curves, Some(vec![[0, 0], [128, 160], [255, 255]]));
        assert_eq!(loaded.crop, Some([1, 2, 3, 4]));
        assert!(loaded.auto_orient);
        assert!(loaded.auto_crop);
        assert_eq!(loaded.infrared_clean.as_deref(), Some("medium"));
        assert!(loaded.descreen);
        assert_eq!(loaded.descreen_dpi, 600);
        assert_eq!(loaded.output_name, "archive");
        assert_eq!(loaded.output_format, "tif");
        assert!(loaded.multipage);
        assert_eq!(loaded.ocr_engine, "tesseract");
        assert_eq!(loaded.ocr_language, "deu");

        save_config(&loaded, &path).unwrap();
        let text = std::fs::read_to_string(&path).unwrap();
        let lower = text.to_ascii_lowercase();
        assert!(!lower.contains("license_key"));
        assert!(!lower.contains("\"drm\""));
        assert!(!lower.contains("activation"));
    }

    #[test]
    fn default_config_path_uses_platform() {
        let p = default_config_path();
        assert!(p.ends_with("config.json"));
        assert!(p.to_string_lossy().contains("open-scanline"));
    }

    #[test]
    fn invalid_persisted_output_and_ocr_choices_are_rejected() {
        for (field, value, expected) in [
            ("output_format", "zip", "output format"),
            ("multipage_format", "zip", "multipage format"),
            ("ocr_engine", "tesserac", "OCR engine"),
    ...
    #[test]
    fn bounded_json_reader_rejects_oversized_regular_files() {
        let path = env::temp_dir().join(format!(
            "open_scanline_config_oversized_{}_{}.json",
            std::process::id(),
        ...
        std::fs::write(&path, vec![b' '; MAX_CONFIG_JSON_BYTES as usize + 1]).unwrap();

        let error = load_config(Some(&path)).unwrap_err();
        assert!(
            matches!(error, ScanError::Invalid(message) if message.contains("1048576-byte limit"))
        );
    ...
    #[cfg(unix)]
    #[test]
    fn bounded_json_reader_rejects_fifo_without_blocking() {
        use std::ffi::CString;
        use std::os::unix::ffi::OsStrExt;
        use std::time::{Duration, Instant};

        let path = env::temp_dir().join(format!(
            "open_scanline_config_fifo_{}_{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        assert!(matches!(error, ScanError::Invalid(message) if message.contains("regular file")));
        std::fs::remove_file(path).unwrap();
    }
}
