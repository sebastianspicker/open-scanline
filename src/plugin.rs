...
#[cfg(test)]
mod tests {
    use super::{plugin_status, run_plugin_mode_inner_with_token, run_plugin_mode_with_token};
    use crate::device::CancellationToken;

    #[test]
    fn plugin_status_backend_availability_is_boolean() {
        let status = plugin_status(None);
        let backends = status["backends"]
            .as_array()
            .expect("plugin status must contain a backend array");
    ...
    #[test]
    fn plugin_scan_rejects_a_corrupt_config_before_acquisition() {
        let directory = std::env::temp_dir().join(format!(
            "open-scanline-plugin-corrupt-config-{}-{}",
            std::process::id(),
        ...
        let error = run_plugin_mode_inner_with_token(
            Some(&config),
            Some(&output),
            true,
            Some("mock"),
    ...
    #[test]
    fn pre_cancelled_plugin_token_prevents_output_and_returns_interrupt_status() {
        let directory = std::env::temp_dir().join(format!(
            "open-scanline-plugin-pre-cancelled-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        let code = run_plugin_mode_with_token(None, Some(&output), true, Some("mock"), token);
