//! Small same-directory atomic file publisher for configuration-like data.
...
/// created.
pub(crate) fn output_paths_alias(left: &Path, right: &Path) -> Result<bool> {
...
pub(crate) fn validate_output_leaf(path: &Path, label: &str) -> Result<()> {
