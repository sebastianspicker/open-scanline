...
use crate::export::{
    apply_export_profile, prepare_export_options, save_final_image, ExportOptions,
};
use crate::imaging::load_image;
use crate::pipeline::histogram;
...
pub fn parse_crop_string(value: &str) -> Result<Option<[i32; 4]>, String> {
    ...
    crate::core::parse_crop_components(value)
        .map(Some)
...
pub fn histogram_for_path(path: impl AsRef<Path>) -> crate::core::Result<Value> {
    histogram(&load_image(path)?)
}

/// Validate a readable image and return the path used by the Open action.
pub fn open_image_file(path: impl AsRef<Path>) -> crate::core::Result<PathBuf> {
    let path = path.as_ref();
    if !path.is_file() {
        return Err(crate::core::ScanError::Other(format!(
            "file not found: {}",
            path.display()
        )));
    }
