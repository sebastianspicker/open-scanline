...
use crate::cli::args::{Commands, OcrEngineArg};
...
fn build_export_options(
    ...
    crate::config::validate_ocr_language(&ocr_language).map_err(|error| error.to_string())?;
...
mod tests {
    use super::{
        build_export_options, password_from_bytes, read_pdf_password, resolve_pdf_password,
        ExportOptionOverrides,
    };
    use crate::cli::args::OcrEngineArg;
    ...
    #[test]
    fn password_file_builds_export_options_without_a_password_in_argv() {
        let path = std::env::temp_dir().join("open_scanline_pdf_password_file.txt");
        fs::write(&path, b"safe-file-password\n").unwrap();
        let export = build_export_options(
            &AppConfig::default(),
            ExportOptionOverrides {
                pdf_password_file: Some(path.clone()),
                ..ExportOptionOverrides::default()
            },
        )
        .unwrap();
        let _ = fs::remove_file(path);

        assert_eq!(export.pdf_password.as_deref(), Some("safe-file-password"));
    }

    #[test]
    fn export_options_inherit_configured_ocr_unless_cli_overrides_it() {
        ...
        let inherited = build_export_options(&config, ExportOptionOverrides::default()).unwrap();
        assert_eq!(inherited.ocr_language, "deu");
        assert_eq!(inherited.ocr_engine, OcrEngine::Tesseract);

        let overridden = build_export_options(
            &config,
            ExportOptionOverrides {
                ocr_language: Some("fra".into()),
                ocr_engine: Some(OcrEngineArg::Offline),
                ..ExportOptionOverrides::default()
            },
        )
        .unwrap();
        assert_eq!(overridden.ocr_language, "fra");
        assert_eq!(overridden.ocr_engine, OcrEngine::Offline);
    }

    #[test]
    fn password_file_rejects_unsafe_or_ambiguous_input() {
        let unsafe_error =
            resolve_pdf_password(Some("argv-password".into()), None, false).unwrap_err();
        assert!(unsafe_error.contains("--allow-insecure-password-argv"));

        let conflict_error = resolve_pdf_password(
            Some("argv-password".into()),
            Some(Path::new("password.txt")),
            true,
        )
        .unwrap_err();
        assert!(conflict_error.contains("conflicts"));
    }

    #[test]
    fn password_file_validation_is_bounded_and_normalizes_one_line_ending() {
        let mut password = b"password\r\n".to_vec();
        assert_eq!(password_from_bytes(&mut password).unwrap(), "password");
        assert!(password.is_empty());

        for invalid in [Vec::new(), b"nul\0password".to_vec(), vec![0xff]] {
            let mut invalid = invalid;
            assert!(password_from_bytes(&mut invalid).is_err());
            assert!(invalid.is_empty());
        }
    }

    #[test]
    fn password_file_rejects_input_larger_than_four_kib() {
        let path = std::env::temp_dir().join("open_scanline_pdf_password_oversized.txt");
        fs::write(&path, vec![b'x'; 4097]).unwrap();
        let error = read_pdf_password(&path).unwrap_err();
        let _ = fs::remove_file(path);
