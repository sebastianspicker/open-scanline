...
#[cfg(test)]
fn validate_pdf_searchable_pages_with_limits(
    ...
    for (index, text) in searchable_pages.iter().enumerate() {
        aggregate = checked_pdf_searchable_text_total_with_limits(
            aggregate,
            text,
            index,
            page_limit,
...
mod tests {
    ...
    #[test]
    fn pdf_writer_keeps_serialized_stream_lengths_and_valid_xref_layout() {
        let path = std::env::temp_dir().join(format!(
            "open_scanline_pdf_layout_{}.pdf",
            std::process::id()
        ));
        let _ = std::fs::remove_file(&path);
        let images = [sample_rgb(2, 1, 10), sample_rgb(1, 2, 220)];

        save_pdf_with_options(&path, &images, &PdfOptions::default()).unwrap();

        let bytes = std::fs::read(&path).unwrap();
        assert!(bytes.starts_with(b"%PDF-1.5"));
        assert!(bytes.ends_with(b"%%EOF"));
        let xref_at = xref_offset(&bytes);
        let xref_stream = &bytes[xref_at..];
        assert!(xref_stream.starts_with(b"10 0 obj\n"));
        assert!(xref_stream
        ...
        let stream_lengths = serialized_page_stream_lengths(&bytes);
        assert_eq!(stream_lengths.len(), 4);
        for (declared, actual) in stream_lengths {
            assert_eq!(declared, actual);
            assert!(actual > 0);
        }
        assert_external_pdf_tools(&path);
        let _ = std::fs::remove_file(path);
    }

    fn sample_rgb(width: u32, height: u32, value: u8) -> ImageBuffer {
        ImageBuffer::new(
            width,
            height,
            crate::core::PixelFormat::Rgb8,
    ...
    #[test]
    fn multipage_pdf_stream_budget_rejects_the_next_page_before_document_growth() {
        let mut document = Document::new();
        let pages_id = document.new_object_id();
        let initial_object_count = document.objects.len();
        let mut builder = PdfPageBuilder {
            document: &mut document,
            context: PdfPageContext {
                pages_id,
                searchable_font: None,
                dpi: 150.0,
            },
            retained_image_stream_bytes: MAX_PDF_RETAINED_IMAGE_STREAM_BYTES,
        };

        let error = builder
            .add_page(1, &sample_rgb(1, 1, 80), None, None)
            .unwrap_err();
        drop(builder);
        ...
        assert_eq!(document.objects.len(), initial_object_count);
    }

    #[test]
    fn cancellation_after_first_transform_preserves_existing_pdf() {
        let directory = std::env::temp_dir().join(format!(
            "open_scanline_pdf_cancel_after_first_{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&directory);
        std::fs::create_dir_all(&directory).unwrap();
        let pages = [directory.join("first.png"), directory.join("second.png")];
        for (index, page) in pages.iter().enumerate() {
            crate::imaging::save_image(page, &sample_rgb(2, 2, index as u8), None, None).unwrap();
        }
        let destination = directory.join("output.pdf");
        std::fs::write(&destination, b"previous PDF output").unwrap();
        let token = CancellationToken::new();
        let canceller = token.clone();
        let mut transforms = 0;

        let error = save_pdf_from_paths_with_options_and_transform_and_cancellation(
            &pages,
            &destination,
            &PdfOptions::default(),
            |image| {
        ...
        assert_eq!(std::fs::read(&destination).unwrap(), b"previous PDF output");
    ...
    #[test]
    fn searchable_pdf_text_budgets_reject_per_page_and_aggregate_overflow() {
        let per_page_error =
            validate_pdf_searchable_pages_with_limits(&["four".into()], 3, 16).unwrap_err();
        assert!(matches!(
            per_page_error,
            ScanError::Invalid(message)
                if message == "searchable PDF page 1 exceeds the 3-byte UTF-8 safety limit"
        ));

        let aggregate_error =
            validate_pdf_searchable_pages_with_limits(&["abc".into(), "de".into()], 3, 4)
                .unwrap_err();
        assert!(matches!(
            aggregate_error,
            ScanError::Invalid(message)
                if message == "searchable PDF text aggregate exceeds the 4-byte UTF-8 safety limit"
        ));
    }

    #[test]
    fn pdf_options_reject_oversized_searchable_page_before_document_creation() {
        let options = PdfOptions {
            searchable_pages: Some(vec!["a".repeat(MAX_PDF_SEARCHABLE_PAGE_UTF8_BYTES + 1)]),
            ..PdfOptions::default()
        };

        let error = validate_pdf_options(1, &options).unwrap_err();

        assert!(matches!(
            error,
            ScanError::Invalid(message)
                if message == format!(
                    "searchable PDF page 1 exceeds the {MAX_PDF_SEARCHABLE_PAGE_UTF8_BYTES}-byte UTF-8 safety limit"
                )
        ));
    }

    fn xref_offset(bytes: &[u8]) -> usize {
        let marker = b"startxref\n";
        let start = bytes
            .windows(marker.len())
            .rposition(|window| window == marker)
            .unwrap();
        let offset_text = &bytes[start + marker.len()..];
        let end = offset_text.iter().position(|byte| *byte == b'\n').unwrap();
        std::str::from_utf8(&offset_text[..end])
            .unwrap()
            .parse()
            .unwrap()
    }

    fn serialized_page_stream_lengths(bytes: &[u8]) -> Vec<(usize, usize)> {
        let mut lengths = Vec::new();
        let mut search_from = 0;
        while let Some(relative_stream) = find_bytes(&bytes[search_from..], b">>stream\n") {
            let stream_at = search_from + relative_stream;
            let dictionary_start = bytes[..stream_at]
                .windows(2)
                .rposition(|window| window == b"<<")
                .unwrap();
            let dictionary = &bytes[dictionary_start..stream_at];
            let stream_start = stream_at + b">>stream\n".len();
            let stream_end =
                stream_start + find_bytes(&bytes[stream_start..], b"\nendstream").unwrap();
            search_from = stream_end + b"\nendstream".len();
            if dictionary
                .windows(b"/Type/XRef".len())
                .any(|window| window == b"/Type/XRef")
            {
            ...
            let relative_length = find_bytes(dictionary, b"/Length ").unwrap();
            let length_start = dictionary_start + relative_length + b"/Length ".len();
            let length_end = bytes[length_start..stream_at]
                .iter()
                .position(|byte| !byte.is_ascii_digit())
                .map(|index| length_start + index)
                .unwrap_or(stream_at);
            let declared = std::str::from_utf8(&bytes[length_start..length_end])
                .unwrap()
                .parse()
                .unwrap();
            lengths.push((declared, stream_end - stream_start));
        }
        lengths
    }
    ...
    fn assert_external_pdf_tools(path: &Path) {
        assert_pdf_tool("pdfinfo", Command::new("pdfinfo").arg(path));

        let render_prefix = path.with_extension("render");
        assert_pdf_tool(
            "pdftoppm",
        ...
        let rendered = render_prefix.with_file_name(format!(
            "{}-1.png",
            render_prefix.file_name().unwrap().to_string_lossy()
        ));
        if rendered.exists() {
    ...
    fn assert_pdf_tool(name: &str, command: &mut Command) {
        match command.output() {
            Ok(output) => assert!(
                output.status.success(),
                "{name} rejected generated PDF: {}",
