...
use crate::core::{ImageBuffer, PixelFormat, Result, ScanError, MAX_IMAGE_BYTES};
use crate::device::CancellationToken;
use crate::imaging::pdf::{save_pdf_with_options_and_cancellation, PdfOptions};
...
pub fn load_image(path: impl AsRef<Path>) -> Result<ImageBuffer> {
    load_image_with_limits(
        path,
        crate::core::MAX_IMAGE_DIMENSION,
        crate::core::MAX_IMAGE_DIMENSION,
        crate::core::MAX_IMAGE_BYTES as u64,
    )
}
...
impl OutputTemp {
    ...
    pub(super) fn publish(mut self) -> Result<()> {
        crate::atomic_write::replace_file_atomic(&self.path, &self.destination)?;
        self.published = true;
        Ok(())
    }
}
