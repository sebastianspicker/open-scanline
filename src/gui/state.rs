...
use crate::config::{
    default_config_path, format_curve_points, load_config, parse_curve_points, validate_hue,
    validate_output_name, validate_saturation, AppConfig,
};
use crate::core::{PipelinePrefs, Rect, Result, Rotate, ScanMode};
use crate::device::list_all_devices;
...
pub(super) struct MultipageSaveCandidate {
    pub(in crate::gui) destination: PathBuf,
    pub(in crate::gui) sources: Vec<PathBuf>,
...
impl GuiState {
    pub(super) fn new(config_path: Option<&Path>) -> Self {
        ...
        Self {
            ...
            device,
            devices,
            width: config.default_width.max(16),
            height: config.default_height.max(16),
            dpi: config.default_dpi.max(crate::core::MIN_SCAN_DPI),
            ...
            hue: config.hue,
            curves_text: format_curve_points(config.curves.as_deref()),
            ...
            grain_reduction: config
                .grain_reduction
                .clone()
                .unwrap_or_else(|| "off".into()),
            flatten: config.flatten,
            hole_punch: config.hole_punch,
            colorize_mode: config.colorize_mode.clone().unwrap_or_else(|| "off".into()),
            film_type: config.film_type.clone().unwrap_or_default(),
            media: media_for_scan_mode(config.scan_mode).into(),
    ...
    pub(super) fn validate_color_controls(&self) -> crate::core::Result<()> {
    ...
    pub(in crate::gui) fn export_options(&self) -> ExportOptions {
        ExportOptions {
...
fn devices_and_selection(config: &AppConfig) -> (Vec<String>, String) {
    let devices: Vec<String> = list_all_devices().into_iter().map(|d| d.id).collect();
