...
#[cfg(feature = "gui")]
use crate::backend_process::TemporaryOutput;
#[cfg(feature = "gui")]
...
impl OpenScanlineApp {
    ...
    pub(super) fn start_scan(&mut self, preview: bool, advance_frame: bool) {
        ...
        self.job = Some(Self::spawn_job(
            ...
            move |sender| {
                ...
                let result: crate::core::Result<PathBuf> = (|| {
    ...
    fn start_save_action(&mut self, action: SaveAction) {
        ...
        self.start_file_job(cancel, pending_pdf_password, "status.saving", move || {
            let result: crate::core::Result<PathBuf> = (|| match action.candidate.as_ref() {
...
mod tests {
    ...
    fn pdf_scan_keeps_a_raster_working_image_for_follow_up_actions() {
        ...
        assert!(crate::imaging::load_image(&working).is_ok());

        app.state.ocr_engine = "offline".into();
        app.start_ocr();
        drain_until_idle(&mut app, &context);
        assert!(app.state.status.contains(crate::ocr::OFFLINE_OCR_ENGINE));
        let _ = std::fs::remove_dir_all(directory);
    }
    ...
    fn write_test_image(path: &Path) {
        crate::imaging::save_image(
            path,
            &crate::core::ImageBuffer::new(1, 1, crate::core::PixelFormat::Rgb8, vec![20, 30, 40])
                .unwrap(),
            None,
            None,
        )
